    ...
    render() {
        return (
            <nav className="navbar navbar-expand-lg navbar-dark" style={{backgroundColor: '#4e4e4e'}}>
                ...
                <SockJsClient url= {WEBURL + 'http://localhost:8080/stomp'} topics={['/topic/'+localStorage.getItem("companyId")+'/markPoint/','/topic/'+localStorage.getItem("companyId")+'/driverArrival/']}
                              onMessage={(msg) => {
                                  this.handleMessage(msg);
                              }}
                              ref={ (client) => { this.clientRef = client }} />
                <SockJsClient url= {WEBURL + 'http://localhost:8080/stomp'} topics={['/topic/'+localStorage.getItem("companyId")+'/editOrder/']}
                              onMessage={(msg) => {
                                  this.handleMessageEditOrder(msg);
                              }}
                              ref={ (client) => { this.clientRef = client }} />
                <SockJsClient url= {WEBURL + 'http://localhost:8080/stomp'} topics={['/topic/'+localStorage.getItem("companyId")+'/createOrder/']}
                              onMessage={(msg) => {
