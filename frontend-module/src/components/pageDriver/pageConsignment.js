...
class DriverConsignment extends Component {
    ...
    setLostState(productId) {
        ...
        let orderId = split[split.length - 1];
        const ref = this;
        fetch(`http://localhost:8080/api/orders/getMyOrders/${orderId}/cancelProduct/${productId}`, {method: "GET", headers: {'Auth-token': localStorage.getItem("Auth-token")}}).then(function (response) {
            return response.json();
        }).then(function (result) {
            console.log(result);
