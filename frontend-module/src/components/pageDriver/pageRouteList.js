...
class DriverRouteList extends React.Component{
    ...
    getRouteList() {
        return fetch('http://localhost:8080/api/orders/getMyOrders/'+this.state.orderId+'/routelist', {method: "get"}).then(function (response) {
            return response.json();
        }).then(function (result) {
            return result;
        });
    ...
    renderTable(routePoint){
        ...
        return <div className = "row table_row order_row">
            ...
            <div className="col-md-3"><a onClick={this.markPoint.bind(this,routePoint.id)} className="table_button bg-secondary text-white">Отметиться</a></div>
        </div>
    }
