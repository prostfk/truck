...
export default class AdminCharts extends Component {
    ...
    constructor(props) {
        ...
        let data = [ [1225929600000, 14.16], [1226016000000, 14.03], [1226275200000, 13.7], [1226361600000, 13.54], [1226448000000, 12.87], [1226534400000, 13.78], [1226620800000, 12.89], [1226880000000, 12.59], [1226966400000, 12.84], [1227052800000, 12.33], [1227139200000, 11.5], [1227225600000, 11.8], [1227484800000, 13.28], [1227571200000, 12.97], [1227657600000, 13.57], [1227830400000, 13.24], [1228089600000, 12.7], [1228176000000, 13.21], [1228262400000, 13.7], [1228348800000, 13.06], [1228435200000, 13.43], [1228694400000, 14.25], [1228780800000, 14.29], [1228867200000, 14.03], [1228953600000, 13.57], [1229040000000, 14.04], [1229299200000, 13.54]];
        let config = {
            rangeSelector: {
                selected: 1
            },
        ...
        this.state = {
            beginDate: CommonUtil.getCorrectDateFromLong(new Date().getTime()),
            endDate: CommonUtil.getCorrectDateFromLong(new Date(new Date().getTime() + 86400000)),
            config: config
        };
        this.fetchToData();
    }
    ...
    fetchToData = () => {
        //todo fetch to server with info
        fetch(`http://localhost:8080/api/getCompanyInfo?dateBegin=11/11/2018&dateEnd=12/11/2018`,{headers:{'Auth-token': localStorage.getItem('Auth-token')}}).then(response=>{
